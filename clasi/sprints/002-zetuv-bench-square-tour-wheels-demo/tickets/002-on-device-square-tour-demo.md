...
- [ ] A demo module exists with one clearly-documented entry point and
...
- [ ] The implementation choice (MoveQueue vs. timed diffdrive.drive,
      per-segment if mixed) is documented in the module's docstring,
      grounded in what `motion.py` actually exposes — not assumed.
- [ ] The demo was run on zetuv; the bench log records the observation
...
- [ ] `python3 -m pytest tests/` stays green (no regressions from
      sprint 001 or ticket 001).
- [ ] `python3 -m py_compile` passes on the new demo module;
